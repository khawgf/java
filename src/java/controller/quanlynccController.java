 ...
 * @author hp
 */
@WebServlet(name = "quanlynccController", urlPatterns = {"/quanlynccController"})
public class quanlynccController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       try {
             quanlynccDAO spcDAO = new quanlynccDAO();
           int currentPage = 1; 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("addNCC".equals(action)) {
            try {
                ...
                String phone = request.getParameter("txtadminphone");
                

                quanlynccDAO dao = new quanlynccDAO();
                if (dao.checkNameNCC(name) == true) {
                    response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                } else {
                    dao.addNhaCungCap(name, phone);
            ...
            try {
                ...
                quanlynccDAO dao = new quanlynccDAO();
                dao.updateNhaCungCap(id, name, phone);
                response.setStatus(HttpServletResponse.SC_OK);
            } catch (Exception e) {
                e.printStackTrace();
                response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
